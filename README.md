[![C# Survival Guide Banner](.github/images/c-sharp-survival-guide.jpg)](https://learn.gamedevhq.com/p/the-unity-c-survival-guide)

# GameDevHQ: Unity C# Suvival Guide

> **Note**: this repository is no longer maintained. The course has been updated and is no longer freely available on the Unity Learn platform.
>
> 3D assets used throughout this course were generously provided by GameDevHQ. However, these assets must now be purchased from [GameDevHQ's Filebase](https://filebase.gamedevhq.com/).
>
> If you do not wish to subscribe to GameDevHQ's filebase, feel free to search for alternative assets on sites such as [Kenny](https://kenney.nl/assets) to use in your scenes. Please be aware that simple GameObjects can be used in place of the 3D assets provided by GameDevHQ.

![Last Updated](https://img.shields.io/badge/June_2021-critical?label=Last%20Updated&style=flat-square)
![Not Maintained](https://img.shields.io/badge/Not_Maintained-critical?label=Status&style=flat-square)

## Course Description:
**This course has been authorized and developed in collaboration with Unity Technologies.**

Have you been struggling to learn C# programming with Unity? If this is the case, you've found the course you've been looking for! This course is intended for programmers at all levels, from beginner to advanced or professional. Sequential completion of the lessons is recommended for beginners, as it will teach you the fundamentals of coding through interactive challenges and problem solving techniques. 

The course progresses in difficulty, and when finished, you will have progressed from a novice C# developer to an advanced, job-ready C# developer. This course is intended for advanced or professional programmers as a reference guide to help you survive in the workplace. What are your thoughts on delegates and events? Check out the section of this course that is entirely dedicated to them and teaches you how to incorporate them into your projects. Sequential progression through the course is not required for experienced programmers; the Unity C# Survival Guide is adaptable, allowing developers to use it for their specific needs.  New content is constantly being added, so you can be confident that your survival guide is always up to date. This is the only C# in Unity programming course that has been authorized and developed in collaboration with Unity Technologies.

## Unity Quick Tips

<details>
<summary>Click to expand</summary>

### Misc. Unity Content
---
* [Change GameObject Position](Assets/Scripts/MiscUnityContent/ObjectPosition/Player.cs)
* [User Input](Assets/Scripts/MiscUnityContent/UserInput/Main.cs)
* [Simple Movement](Assets/Scripts/MiscUnityContent/SimpleMovement/Player.cs)
* [Collectable GameObjects](Assets/Scripts/MiscUnityContent/Collectable/Collectable.cs)
* [Pause Systems](Assets/Scripts/MiscUnityContent/PauseSystem/Pause.cs)
* [Destructable Crate](Assets/Scripts/MiscUnityContent/DestructableObject/Crate.cs)
* [Quaternion and Euler Angles](Assets/Scripts/MiscUnityContent/Rotation/SpawnCube.cs)
* [Quaternion Look Rotation](Assets/Scripts/MiscUnityContent/Rotation/Aim.cs)
* [Quaternion Slerp](Assets/Scripts/MiscUnityContent/Rotation/SmoothAim.cs)

</details>

## Biginner C#

<details>
<summary>Click to expand</summary>

### Variables
---
* [Hands-on Variables](Assets/Scripts/01_Variables/Variables.cs)
* [Common Data Types](Assets/Scripts/01_Variables/CommonDataTypes.cs)

#### Challenges
* [Master Variables, part 1](Assets/Scripts/01_Variables/Challenge01/MasterVars_01.cs)
* [Master Variables, part 2](Assets/Scripts/01_Variables/Challenge02/MasterVars_02.cs)
* [Tip Calculator](Assets/Scripts/01_Variables/Challenge03/TipCalculator.cs)
* [Quiz Grade Average](Assets/Scripts/01_Variables/Challenge04/QuizCalulator.cs)

### If-Then Logic
---
* [If-Statements](Assets/Scripts/02_If-Then/Player_If.cs)
* [Else-If Statements](Assets/Scripts/02_If-Then/Player_ElseIf.cs)

#### Challenges
* [Master If-Statements, part 1](Assets/Scripts/02_If-Then/Challenge01/MasterIf_01.cs)
* [Master If-Statements, part 2](Assets/Scripts/02_If-Then/Challenge02/MasterIf_02.cs)
* [Master If-Statements, part 3](Assets/Scripts/02_If-Then/Challenge03/MasterIf_03.cs)
* [Quiz Grades](Assets/Scripts/02_If-Then/Challenge04/QuizGrades.cs)
* [Master If-Statements, part 4](Assets/Scripts/02_If-Then/Challenge05/MasterIf_04.cs)
* [Speed Program](Assets/Scripts/02_If-Then/Challenge06/Speed.cs)

### Switch Statements
---
* [Hands-on Switch Statements](Assets/Scripts/03_Switch/Switch.cs)

#### Challenges
* [Master Switch Statements, part 1](Assets/Scripts/03_Switch/Challenge01/MasterSwitch_01.cs)
* [Master Switch Statements, part 2](Assets/Scripts/03_Switch/Challenge02/MasterSwitch_02.cs)
* [Weapon Select Program](Assets/Scripts/03_Switch/Challenge03/WeaponSelect.cs)

### Loops
---
* [For Loops](Assets/Scripts/04_Loops/ForLoops_01.cs)
* [For Loops, continued](Assets/Scripts/04_Loops/ForLoops_02.cs)
* [Break Out of Loops](Assets/Scripts/04_Loops/LoopBreak.cs)
* [Do While Loop](Assets/Scripts/04_Loops/DoWhile.cs)
* [While Loop](Assets/Scripts/04_Loops/WhileLoop.cs)

#### Presentations
* [For Loop Presentation](Assets/Scripts/04_Loops/Presentations/ForLoop.cs)

#### Challenges
* [Ultimate Print Out](Assets/Scripts/04_Loops/Challenge01/UltimatePrintOut.cs)
* [How Fast Are You Going?](Assets/Scripts/04_Loops/Challenge02/SpeedCheck.cs)

### Arrays
---
* [Array Syntax](Assets/Scripts/05_Arrays/Syntax.cs)
* [Access Array Elements](Assets/Scripts/05_Arrays/Elements.cs)
* [Print All Elements Using For Loops](Assets/Scripts/05_Arrays/PrintArray_01.cs)
* [Print All Elements Using Foreach Loops](Assets/Scripts/05_Arrays/PrintArray_02.cs)

#### Presentations
* [Looping Through Data](Assets/Scripts/05_Arrays/Presentations/ArrayLooping.cs)
* [Looping Practical Example](Assets/Scripts/05_Arrays/PracticalExample_01.cs)

#### Challenges
* [Array Master](Assets/Scripts/05_Arrays/Challenge01/MasterArrays.cs)
* [Randomly Choose One](Assets/Scripts/05_Arrays/Challenge02/SelectRandom.cs)
* [Convert to Foreach Loop](Assets/Scripts/05_Arrays/Challenge03/PracticalExample_02.cs)

### Functions vs Methods
---
* [Method Parameters](Assets/Scripts/06_Functions_Methods/Parameters.cs)
* [Return Type Functions](Assets/Scripts/06_Functions_Methods/ReturnType.cs)
* [Change Positions 4 Ways](Assets/Scripts/06_Functions_Methods/ChangePosition.cs)

#### Presentations
* [Return Array](Assets/Scripts/06_Functions_Methods/Presentations/ReturnArray.cs)

#### Challenges
* [Pass the Object with Color](Assets/Scripts/06_Functions_Methods/Challenge01/ObjectAsParameter.cs)
* [Are You Alive?](Assets/Scripts/06_Functions_Methods/Challenge02/PlayerStatus.cs)
* [Position Matters](Assets/Scripts/06_Functions_Methods/Challenge03/GeneratePosition.cs)

### Classes
---
* [Classes for Behaviors](Assets/Scripts/07_Classes)
* [Custom Classes](Assets/Scripts/07_Classes/CustomClasses)
* [Class Inheritence](Assets/Scripts/07_Classes/Inheritence)
* [Virtual Methods and Overriding](Assets/Scripts/07_Classes/VirtualMethods_Overriding)
* [Structs, Memory Management, Values and Reference Types](Assets/Scripts/07_Classes/Structs/Test.cs)

#### Presentations
* [Serialized Custom Class RPG Item Database](Assets/Scripts/07_Classes/Presentations/RPG_ItemDB)
* [RPG Spell System](Assets/Scripts/07_Classes/Presentations/RPG_Spells)
* [Bank System Inheritence](Assets/Scripts/07_Classes/Presentations/BankSystem)

#### Challenges
* [Customer Database](Assets/Scripts/07_Classes/Challenge01)

### Static Types
---
* [Declaring Static Types](Assets/Scripts/08_StaticTypes)
* [Instance Members vs Static Members](Assets/Scripts/08_StaticTypes/Instance_Static/Test.cs)
* [Utility Helper Classes](Assets/Scripts/08_StaticTypes/UtilityHelper)
* [Initializing Static Members with a Static Constructor](Assets/Scripts/08_StaticTypes/Constructor)

#### Presentations
* [Working with Static Types](Assets/Scripts/08_StaticTypes/Presentations)

#### Challenges
* [Random Color Helpers](Assets/Scripts/08_StaticTypes/Challenge01)

</details>

## Intermediate C#

<details>
<summary>Click to expand</summary>

### Properties
---
* [Hands-on Properties](Assets/Scripts/09_Properties/GameManager.cs)
* [Auto Properties](Assets/Scripts/09_Properties/AutoProperties/GameManager.cs)

#### Challenges
* [Declaring Properties](Assets/Scripts/09_Properties/Challenge01/GameManager.cs)

### Namespaces
---
* [Hands-on Namespaces](Assets/Scripts/10_Namespaces/Weapon.cs)

### Lists
---
* [Hands-on Lists](Assets/Scripts/11_Lists/GameManager.cs)

#### Presentations
* [Creating Inventory Item DB Systems](Assets/Scripts/11_Lists/Presentations)

#### Challenges
* [You Have Been Named](Assets/Scripts/11_Lists/Challenge01/GameManager.cs)
* [Build That Object List](Assets/Scripts/11_Lists/Challenge02/GameManager.cs)

### Enums
---
* [Hands-on Enums](Assets/Scripts/12_Enums/SelectDifficulty.cs)
* [Enum Enemy AI](Assets/Scripts/12_Enums/EnemyAI/EnemyAI.cs)
* [Enums in Custom Classes](Assets/Scripts/12_Enums/CustomClass)
* [Casting Enums to Ints](Assets/Scripts/12_Enums/CastToInt/LevelSelect.cs)

### Dictionaries
---
* [Hands-on Dictionaries](Assets/Scripts/13_Dictionaries)
* [Looping Through Dictionary](Assets/Scripts/13_Dictionaries/Looping)
* [Dictionary for Player Connections](Assets/Scripts/13_Dictionaries/PlayerConnections)
* [Dictionary with Primitive Types](Assets/Scripts/13_Dictionaries/PrimitiveTypes/Main.cs)

</details>

## Advanced C#

<details>
<summary>Click to expand</summary>

### Abstract Classes and Interfaces
---
* [Abstract Class and Methods](Assets/Scripts/14_AbstractClasses_Interfaces/Enemy.cs)
* [Interfaces Made Easy](Assets/Scripts/14_AbstractClasses_Interfaces/Interfaces.cs)
* [What is Polymorphism?](Assets/Scripts/14_AbstractClasses_Interfaces/Polymorphism.cs)
* [Generic Interfaces](Assets/Scripts/14_AbstractClasses_Interfaces/GenericInterfaces.cs)

#### Challenges
* [Employee Experience](Assets/Scripts/14_AbstractClasses_Interfaces/Challenge01)

### Delegates and Events
---
* [Delegates](Assets/Scripts/15_Delegates_Events/Delegates/Main.cs/)
* [Events](Assets/Scripts/15_Delegates_Events/Events)
* [Actions](Assets/Scripts/15_Delegates_Events/Actions)
* [Return Type Delegates and Func](Assets/Scripts/15_Delegates_Events/Func/Main.cs)
* [Void Delegates with Parameters](Assets/Scripts/15_Delegates_Events/Exercises/Practice01)
* [Void Delegates with No Parameters](Assets/Scripts/15_Delegates_Events/Exercises/Practice02)
* [Return Delegates with Parameters](Assets/Scripts/15_Delegates_Events/Exercises/Practice03)
* [Retrn Delegate with No Parameters](Assets/Scripts/15_Delegates_Events/Exercises/Practice04)
* [Simple Callback System](Assets/Scripts/15_Delegates_Events/CallbackSystem/Main.cs)

#### Presentations
* [Working with Delegates and Events](Assets/Scripts/15_Delegates_Events/Delegates_Events)
* [Practical Event Driven Programming](Assets/Scripts/15_Delegates_Events/EventDriven)

#### Challenges
* [Teleport Events](Assets/Scripts/15_Delegates_Events/Challenge01)

### LINQ: Language-Integrated Query
---
* [LINQ Introduction: Any](Assets/Scripts/16_LINQ/LinqAny.cs)
* [LINQ: Contains](Assets/Scripts/16_LINQ/LinqContains.cs)
* [LINQ: Distinct](Assets/Scripts/16_LINQ/LinqDistinct.cs)
* [LINQ: Where](Assets/Scripts/16_LINQ/LinqWhere.cs)
* [LINQ: Order by Descending](Assets/Scripts/16_LINQ/LinqDescending.cs)
* [How to Read and Convert Query Syntax](Assets/Scripts/16_LINQ/QuerySyntax.cs)

#### Challenges
* [Hands on with LINQ](Assets/Scripts/16_LINQ/Challenge01/QuizFilter.cs)
* [Filter Items](Assets/Scripts/16_LINQ/Challenge02/ItemFilter.cs)

### Design Patterns: Singleton
---
* [Singleton Design Pattern](Assets/Scripts/DesignPatterns/17_Singleton)
* [Singleton UI Manager](Assets/Scripts/DesignPatterns/17_Singleton/UIManager)
* [Singleton Lazy Instantiation](Assets/Scripts/DesignPatterns/17_Singleton/LazyInstantiation)
* [Downside of Lazy Instantiation](Assets/Scripts/DesignPatterns/17_Singleton/LazyInstantiation/Downside)
* [MonoSingleton](Assets/Scripts/DesignPatterns/17_Singleton/MonoSingleton)

#### Challenges
* [Singleton Spawn Manager](Assets/Scripts/DesignPatterns/17_Singleton/Challenge01)

### Design Patterns: Object Pool
---
* [Object Pooling Design Pattern](Assets/Scripts/DesignPatterns/18_ObjectPool)

#### Challenges
* [Pool Manager](Assets/Scripts/DesignPatterns/18_ObjectPool/Challenge01)
* [Request from Pool Manager](Assets/Scripts/DesignPatterns/18_ObjectPool/Challenge02)
* [Recycle the Pool](Assets/Scripts/DesignPatterns/18_ObjectPool/Challenge03)

### Design Patterns: Command Pattern
---
* [Command Pattern Implementation](Assets/Scripts/DesignPatterns/19_CommandPattern/CommandPattern)

#### Presentations
* [Practical Command Pattern](Assets/Scripts/DesignPatterns/19_CommandPattern/Presentation)

#### Challenges
* [The Command Manager](Assets/Scripts/DesignPatterns/19_CommandPattern/Challenge01)

</details>

## Assets
The following Scenes will contain `Missing Prefabs` without the assets provided by GameDevHQ:

+ ### 07_Classes:
  - Classes
  - Custom Classes
  
+ ### MiscUnityContent
  - Pause Systems
  - Post Processing Effects
  - Destructable Crate
  - Quaternion Look Rotation
  - Quaternion Slerp

## Built With
![Unity 2020.3](https://img.shields.io/badge/Unity-2020.3-informational?logo=unity&labelColor=000&style=flat-square)
![C#](https://img.shields.io/badge/C%20Sharp-000?style=flat-square&logo=data:image/png;base64,)
![GameDevHQ: C# Survival Guide](https://img.shields.io/badge/C_Sharp_Survival_Guide-1794ab?labelColor=000&label=GameDevHQ&style=flat-square&logo=data:image/png;base64,)

## License
This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

##### "Unity", Unity logos, and other Unity trademarks are trademarks or registered trademarks of Unity Technologies or its affiliates in the U.S. and elsewhere. Other names or brands are trademarks of their respective owners.
